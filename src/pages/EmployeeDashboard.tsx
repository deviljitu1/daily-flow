...
const EmployeeDashboard = () => {
  ...
  const myTasks = tasks.filter(t => t.user_id === user?.userId); // Assuming tasks have role or just filtering by user_id

  // Actually context already filters by user_id for employees, so `tasks` should be correct if backend logic holds.
  // But let's be safe. Wait, context says: if user.role !== 'admin', query = query.eq('user_id', user.userId);
  // So `tasks` in context are already filtered for the employee.
