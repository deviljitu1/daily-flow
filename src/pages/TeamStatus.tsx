...
const TeamStatus = () => {
    ...
    // Finished: No active task, AND has completed tasks, AND has NO pending todo/progress tasks
    const finishedMembers = teamActivity.filter(m =>
        !m.current_task_title &&
        (!m.todo_tasks || m.todo_tasks.length === 0) &&
        (!m.progress_tasks || m.progress_tasks.length === 0) &&
        (m.completed_tasks && m.completed_tasks.length > 0)
    );
